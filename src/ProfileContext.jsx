...
export function calculateProfileCompleteness(profile) {
  ...
  let score = 0;
  fields.forEach((f) => {
    if (f && String(f).trim()) score += 6;
  });
  arrays.forEach((a) => {
    if (Array.isArray(a) && a.length > 0) score += 8;
  });
...
export function ProfileProvider({ children }) {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (profiles.length > 0) {
        ...
        console.log(
          "✅ ProfileContext: created new profile in API, id =",
          created.id,
        );
      }
    } catch (err) {
      console.warn(
        "⚠️ ProfileContext: API failed, using localStorage fallback:",
        err.message,
      );
      _loadFromLocalStorage();
    }
  ...
  const _saveProfile = async (data) => {
    ...
    try {
      ...
      console.warn(
        "⚠️ ProfileContext: API save failed, localStorage used:",
        err.message,
      );
    }
  };
